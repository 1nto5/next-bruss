...
import type { LoginType, HydraScanType, PalletScanType } from './lib/zod';
import type { HydraScanResult, PalletScanResult, ArticleStatus } from './lib/types';
...
export async function login(data: { identifier1: string; identifier2?: string; identifier3?: string }) {
  try {
...
export async function saveHydraBatch(hydraQr: string, operator: string): Promise<HydraScanResult> {
  try {
    // Validate QR format
    ...
    const qrArticle = splitHydraQr[0]?.length === 7 ? splitHydraQr[0].substring(2) : null;
    ...
    // Validate quantity
    const qrQuantity = splitHydraQr[2] ? parseInt(splitHydraQr[2].substring(2)) : 0;
...
export async function savePalletBatch(
  ...
  try {
    ...
    const qrArticle = splitPalletQr[0]?.length === 7 ? splitPalletQr[0].substring(2) : null;
...
export async function printPalletLabel(
  ...
  try {
    ...
    console.log(`Printing label for ${articleName} (${articleNumber}) at ${workplace}`);

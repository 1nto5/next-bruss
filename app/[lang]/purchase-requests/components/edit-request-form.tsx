...
import { submitPurchaseRequest, updatePurchaseRequest } from '../actions';
...
export default function EditRequestForm({
  ...
  async function onSubmit(data: FormData, shouldSubmit: boolean) {
    if (!shouldSubmit) setSavingDraft(true);

    startTransition(async () => {
      const result = await updatePurchaseRequest(request._id, {
      ...
      // If shouldSubmit and it's a draft, submit it
      if (shouldSubmit && request.status === 'draft') {
        const submitResult = await submitPurchaseRequest(request._id);
      ...
      setSavingDraft(false);
      router.push(`/${lang}/purchase-requests/${request._id}`);
      router.refresh();
    });
  }
    ...
    <Form {...form}>
      <form className='space-y-6'>
        <Card>
          <CardHeader>
            <CardTitle>
              {dict.form.editTitle} {request.internalId || dict.details.draft}
            </CardTitle>
            <CardDescription>
              Edytuj dane zapotrzebowania zakupowego.
            </CardDescription>
          </CardHeader>
